<script lang="ts">
    ...
    const handleMouseMove = (event:MouseEvent) => {
        position.x = event.layerX || 0
        position.y = event.layerY || 0
        updateRotation()
    }
...
<div
    ...
    <div bind:this={card} class="h-full flex justify-center">  
        <div class="w-full -z-10 bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90%">
            <img src={data.framed_demo} class="object-cover w-full h-full -z-10" alt="">
            <div class="">
                {data.description}
                {data.name}
            </div>
