<script lang="ts">
	...
	const updateRotation = () => {
		...
		const extractRotation = (axis: string) =>
			parseFloat((transform.match(new RegExp(`${axis}\\(([^)]+)deg\\)`)) || [])[1]) || 0;
		const xRot = extractRotation('rotateX');
		const yRot = extractRotation('rotateY');
		const yRotation =
			Math.floor(lerp(yRot, 10 * ((position.x - width / 2) / width), 0.1) * 100) / 100;
		const xRotation =
			Math.floor(lerp(xRot, -10 * ((position.y - height / 2) / height), 0.1) * 100) / 100;
		const string =
			'perspective(1000px) rotateX(' + xRotation + 'deg) rotateY(' + yRotation + 'deg)';
...
>
	<div bind:this={card} class="card h-full flex justify-center">
		...
		>
			...
			<a
				class="absolute bottom-0 w-full h-full md:opacity-0 hover:opacity-100 active:opacity-100 z-10 transition-opacity"
				href={`${data.path}`}
			>
				<div
					class="absolute bottom-0 h-24 w-full flex flex-col bg-gray-900 bg-opacity-50 text-white p-4"
				>
					<div class="flex flex-row divide-x divide-slate-800 uppercase -ml-4">
						{#each data.keywords || [] as keyword}
							<div class="px-4 text-sm lg:text-lg">{@html keyword}</div>
						{/each}
					</div>
					<h2 class="text-xl lg:text-3xl">{data.name}</h2>
				</div>
			</a>
		</div>
	</div>
